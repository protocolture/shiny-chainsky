...
pygame.mixer.set_num_channels(18)  # 16 base + 2 for override

channels = []
track_names = []

# Interleaved radio playlist: [signal, static, signal, static, ...]
playlist = []
for i in range(8):
    ...
    while True:
        ...
        if not override_active and now - last_trigger_time > 60:
            ...
            try:
                ...
                print(f"[ERROR] Failed to load override tracks: {e}")
                override_active = False
                continue

            # Stop base audio after loading
            for ch in channels:
                ch.set_volume(0.0)

            # Play both override tracks
            ...
            # Set override to end when shorter one finishes
